...
import { adizRectCorners, foxhoundAdizArea, isFoxhound } from "../simulator/interceptScenario.js";
...
export function WeatherOverlay({ weatherOn, weatherCells, zoom }) {
  return weatherOn ? weatherCells.map((c) => (
    <g key={c.id}>
      {weatherTiles(c).map((t, idx) => <rect key={`${c.id}-${idx}`} x={t.x} y={t.y} width={t.size} height={t.size} fill={c.level === "RED" ? "#ef4444" : "#facc15"} opacity={t.opacity} />)}
      <text x={c.x} y={c.y} fill={c.level === "RED" ? "#fecaca" : "#fef3c7"} fontSize={10 / zoom} textAnchor="middle">{c.id} {Math.round(c.baseAlt ?? 0)}-{Math.round(c.topAlt ?? 60000)}FT</text>
    </g>
  )) : null;
}
...
export function RadarHeaderOverlay({ viewCenter, viewSize, zoom, seat, activeRunway, windObj, qnh, radarSweepAgeSec, snowRemovalNotice, rjcjPriorityNotice, fmt3 }) {
  return <>
    ...
    {rjcjPriorityNotice ? (() => {
      const color = rjcjPriorityNotice.level === "FAIL" ? "#ef4444" : rjcjPriorityNotice.level === "DANGER" ? "#f97316" : rjcjPriorityNotice.level === "WARN" ? "#facc15" : rjcjPriorityNotice.level === "DONE" ? "#22c55e" : "#38bdf8";
...
export function RadarRunwayOverlay({ runwayDisplaySet }) {
  return <>
    {runwayDisplaySet.map(({ runway: rw, role }) => {
      ...
      const color = role === "CURRENT" ? "#22c55e" : role === "BOTH" ? "#84cc16" : role === "DEP" ? "#a855f7" : role === "PENDING" ? "#f59e0b" : role === "CLOSED" ? "#ef4444" : "#38bdf8";
      return <polygon key={`radar-rwy-${rw}`} points={pts} fill={color} opacity={role === "CURRENT" ? "0.30" : role === "BOTH" ? "0.25" : role === "DEP" ? "0.20" : role === "PENDING" ? "0.18" : role === "CLOSED" ? "0.08" : "0.10"} stroke={color} strokeWidth={role === "CURRENT" ? "1.45" : "1.05"} />;
    })}
  </>;
}
...
export function MissionOverlay({ activeCorridors, aircraft, scenarioId, scenarioEventsDone, zoom, lang }) {
  return <>
    ...
    {displayedMissionAreas(aircraft).map((area) => {
      ...
      const color = area.dynamic ? "#f59e0b" : "#60a5fa";
...
export function TargetLayer({ radarDisplayTargets, selectedId, conflictIds, cautionIds, zoom, selectAircraft, fmt3, fmtFL, vnavStatus, env, modeText }) {
  return <>
    {radarDisplayTargets.map((a) => {
      ...
      const color = conflictIds.has(a.id) ? "#ff1f1f" : cautionIds.has(a.id) ? "#f59e0b" : a.emergency ? "#ff4d4d" : lowFuel ? "#ff4d4d" : isSel ? "#f6e94d" : isFoxhound(a) ? "#ef4444" : a.category === "MIL" ? "#60a5fa" : a.category === "DEP" ? "#c084fc" : a.clearedILS ? "#4de1ff" : a.mode === "HOLD" ? "#f59e0b" : "#32ff4d";
