...
export default function RjccProcedureTraceEditor() {
  ...
  const applyManualPreviewPreset = () => {
    ...
    setTraceType(preset.traceType);
    setOriginAnchorId(preset.anchorFrame.originId);
    setAxisTargetAnchorId(preset.anchorFrame.axisToId);
    setStartAnchorId(preset.anchorFrame.startId);
    setFinalAnchorId(preset.anchorFrame.finalId);
    setConstructionStationId(preset.constructionDefaults.stationId || "CHE");
    if (Number.isFinite(preset.constructionDefaults.radialDeg)) setRadialDegInput(preset.constructionDefaults.radialDeg);
    setBearingType(preset.constructionDefaults.bearingType || "MAGNETIC");
    setMagneticVariationDeg(preset.constructionDefaults.magneticVariationDeg ?? DEFAULT_MAGNETIC_VARIATION_DEG);
    if (preset.constructionDefaults.dmeNm?.length) setDmeRadiusNmInput(preset.constructionDefaults.dmeNm[0]);
